...
import datetime
import math
auth('13627645060','Wsm110630')


# In[2]:


# 获取昨日最高价、最低价、收盘价、开盘价四个数据
df_AP = get_price('AP2304.XZCE',start_date='2022-04-19',end_date='2022-5-19',frequency='1m')


# In[3]:


df_AP.to_csv("D:\Desktop\登辉quant\菲阿里四价\部分数据.csv")
...
# 四价：昨日最高价=阻力线，昨日最低价=支撑线，昨日收盘价，今日开盘价
# 若今日开盘价在阻力线和支撑线之间，价格向上突破阻力线做多，价格向下突破支撑线做空

# In[5]:


def falsj(data,initial_cash,max_lot):
#初始化当前天数、总资产、活动金额、手数(做空为负，做多为正）
    ...
    if n2 == 0:
        ...
        low = data[data.index < data.index[n1] + datetime.timedelta(days = 1)]['low'].min()
